...
$query = "SELECT * FROM `data` ORDER BY `createdAt` DESC LIMIT 100";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $jsonArrayItem['label'] = $row['createdAt'];
        //append the above created object into the main array.
        array_push($labelArray, $jsonArrayItem);
    }

    array_push($dataArray, array_reverse($labelArray));
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $jsonArrayItem = array();
        $jsonArrayItem['value'] = $row['accX'];
        //append the above created object into the main array.
        array_push($xAxisArray, $jsonArrayItem);
    }

    array_push($dataArray, array_reverse($xAxisArray));
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $jsonArrayItem = array();
        $jsonArrayItem['value'] = $row['accY'];
        //append the above created object into the main array.
        array_push($yAxisArray, $jsonArrayItem);
    }

    array_push($dataArray, array_reverse($yAxisArray));
